"""
Verify SWMM junctions attribute column order using a given model folder.

Usage:
    python scripts/verify_junctions_order.py "C:\\path\\to\\model_folder"

Outputs ordered column list and a preview of the attribute table for junctions,
...
def main(folder: str) -> int:
    ...
    junc_gdf = swmm_data.get('junctions')
    if junc_gdf is None or junc_gdf.empty:
        print("ERROR: No junctions geometry extracted from SWMM.inp")
        return 5

    rpt = extract_swmm_junctions_rpt(folder)
    merged_results = rpt.get('merged_results', pd.DataFrame())
    junc_summary = canonicalize_junctions_summary(merged_results)

    # Merge on normalized name
    jdf = junc_gdf.copy()
    if junc_summary is not None and not junc_summary.empty:
        sdf = junc_summary.copy()
        ...
        merged = jdf.merge(sdf, on='_key', how='left')
    ...
    enhanced = apply_junction_schema(merged)
    ...
    with pd.option_context('display.max_columns', None, 'display.width', 180, 'display.max_colwidth', 40, 'display.precision', 4):
        print(preview)

    try:
        gpkg_path = os.path.join(folder, 'swmm_junctions.gpkg')
        if enhanced.crs is None:
            enhanced.set_crs(epsg=EPSG, inplace=True)
        enhanced.to_file(gpkg_path, layer='junctions', driver='GPKG')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python scripts/verify_junctions_order.py <model_folder>")
        sys.exit(1)
