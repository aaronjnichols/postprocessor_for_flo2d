"""QGIS popup dialogs for channel segment and cross-section plots."""
...
class SeriesPlotPanel(QWidget):
    """Reusable checkbox + matplotlib panel for multi-series line plots."""
    ...
    def __init__(
        ...
        self._show_mode_toggle = bool(show_mode_toggle)
        ...
        self._active_series_ids = {
            item["id"] for item in self._series if item["default_on"]
        }
    ...
    def _on_mouse_move(self, event):
        ...
        lines = [f"{self._x_label}: {snap_x:.2f}"]
        anchor_point = None
...
class ChannelCrossSectionDialog(QDialog):
    """Tabbed dialog for cross-section geometry and hydrograph time series."""

    def __init__(self, title, geometry_series, hydrograph_series, stats, parent=None):
        super().__init__(parent)
        ...
        self.geometry_panel = SeriesPlotPanel(
            geometry_series,
            x_label="Station (ft)",
        ...
        geometry_layout.addWidget(self.geometry_panel)
        tabs.addTab(geometry_tab, "Geometry")

        hydrograph_tab = QWidget()
        hydrograph_layout = QVBoxLayout(hydrograph_tab)
        hydrograph_layout.setContentsMargins(0, 0, 0, 0)
        self.hydrograph_panel = SeriesPlotPanel(
            hydrograph_series,
            x_label="Time (hours)",
            y_label=None,
            show_mode_toggle=True,
            parent=hydrograph_tab,
        )
        hydrograph_layout.addWidget(self.hydrograph_panel)
        tabs.addTab(hydrograph_tab, "Time Series")
